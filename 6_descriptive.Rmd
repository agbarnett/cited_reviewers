...
rlabels = c('Approve','Approve with reservations','Reject')
rlabels = c('Approve','Reservations','Reject') # shorter version
...
to_pred = expand.grid(recommendation = c('Approve','Reservations','Reject'),
...
tab = tidy(model, conf.int = TRUE, exponentiate = TRUE) %>%
  ...
  mutate(term = case_when(str_detect(term, 'version') ~ 'Version > 1',
                          ...
                          str_detect(term, 'recommendationReject$') ~ 'Reject',
                          str_detect(term, '^recommendationReservations') ~ 'Reservations x self-cited',
                          str_detect(term, '^recommendationReject') ~ 'Reject x self-cited',
