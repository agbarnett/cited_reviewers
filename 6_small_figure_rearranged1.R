...
limits_odds = c(0.5, 2.2)
limits_probability = c(0.3, 0.64)
...
## get the right data
# estimates
for_plot = filter(results1_only_referees$results, 
                  ...
                  type!= 'linear')
# predictions
for_plot_pred = filter(results1_only_referees$predictions, 
...
plot_top_left = ggplot(data = filter(for_plot, outcome == 1), 
  ...
  scale_y_log10(breaks=c(0.7,1,1.5,2), labels=c('0.7','1','1.5','2.0'))+
  theme_bw()+
...
#
jpeg('figures/6_reviewer_cited_withp.jpg', width = 6.8, height = 6.5, units = 'in', res = 500)
