# 6_small_figure_rearranged3.R
# remake small version of the figure outside of function
# re-arranged to group together results by the outcome categories
# results for non self-cited
# called by 6_models_not_cited.R
# September 2025

# use same scales in upper and lower panels
limits_odds = c(0.1, 1.01)
limits_probability = c(0, 0.54)

## get the right data
# estimates
for_plot = filter(results3_only_referees$results, 
                  ...
                  type!= 'linear')
# predictions
for_plot_pred = filter(results3_only_referees$predictions, 
...
plot_top_left = ggplot(data = filter(for_plot, outcome == 1), 
 ...
 # geom_label(data = text1, aes(x=model, y=y, label=label), size=3, adj=0, col='grey66')+ # turned off as axis is all below 1
...
#
jpeg('figures/6_non_self_cited_withp.jpg', width = 6.8, height = 6.5, units = 'in', res = 500)
