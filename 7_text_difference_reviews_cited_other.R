# 7_text_differences_reviews_cited_other.R
# using text analysis to look for differences in reviews with citations to other work
# September 2025
library(dplyr)
library(stringr)
...
# subset of citations to other work, with not approved as outcome
matched_other = mutate(matched,
  ...
  filter(non_self_cited_count > 0)
# same x design as above, but in subset
x = matrix(data = 0, nrow = nrow(matched_other), ncol = 100)
for (k in 1:100){
  ...
  x[,k] = str_detect(matched_other$rtext, this_feature) 
}
summary(x); any(colMeans(x)==0) # checks
y = matched_other$recommendation != 'approve' # binary outcome not approved
# run elastic net
fit_other = glmnet(x, y, family='gaussian', alpha = 0.95) # data is large enough to use Gaussian
cvfit_other = cv.glmnet(x, y, family='gaussian', alpha = 0.95)
plot(cvfit_other)
c3 = coef(cvfit_other, s = "lambda.1se")
# extract non-zero coefficients
to_plot = data.frame(vars = top_100$feature[c3@i], coefs = c3@x[-1]) %>% # not intercept
...
plot_plus = ggplot(data = to_plot_plus, aes(x = x, y = mean, ymin=lower, ymax=upper)) +
  ...
  geom_errorbar(width=0,col='darkorchid3')+
  geom_point(col='darkorchid3')+
  geom_hline(lty=2, yintercept=0)+
  geom_label(data = text1, aes(x=x, y=y, label=label), size=2.5, adj=0, col='grey50')+
  geom_label(data = text2, aes(x=x, y=y, label=label), size=2.5, adj=1, col='grey50')+
...
plot_plus
# export
jpeg('figures/7_text_differences_approve_cited_other_plus.jpg', width = 5, height = 5, units = 'in', res = 500)
print(plot_plus)
dev.off()
...
save(to_plot_plus, file='results/7_text_other_cited.RData')
